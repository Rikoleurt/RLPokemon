...
def moving_pokemon_move_usage(env: PokemonEnv) -> dict[str, dict[str, list[float]]]:
    ...
    histories = tracker.pokemon_move_counter_history_by_episode
    if not histories:
        return {}

    pokemon_names = list(tracker.pokemon_move_counter.keys())
...
def plot_global_performance(env: PokemonEnv, plot_dir: str) -> None:
    tracker = env.stats_tracker
    episodes = np.arange(1, len(tracker.winrate_history) + 1)
    if episodes.size == 0:
        return
    ...
    axes[0].plot(episodes, tracker.winrate_history, label="Winrate cumule", linewidth=1.8)
    axes[0].plot(
        episodes,
        tracker.winrate_moving_history,
    ...
    fight_lengths = np.asarray(tracker.fight_length_history, dtype=float)
    if fight_lengths.size:
        ...
        axes[1].scatter(
            ...
            s=10,
            alpha=0.25,
            label="Longueur observee",
        )
        axes[1].plot(
            fight_episodes,
            tracker.fight_length_moving_history[:fight_lengths.size],
        ...
        axes[1].text(0.5, 0.5, "Aucune donnee", ha="center", va="center")
    ...
    if pokemon_usage:
        ...
        usage_axes[0].set_title("Usage glissant des attaques par Pokemon")
        usage_axes[0].text(0.5, 0.5, "Aucune donnee par Pokemon", ha="center", va="center")
...
def plot_pokemon_behavior(env: PokemonEnv, plot_dir: str) -> None:
    tracker = env.stats_tracker
    if not tracker.pokemon_move_counter:
        return
    ...
    all_move_names = sorted({
        move_name
        for counter in tracker.pokemon_move_counter.values()
    ...
    for ax, pokemon_name in zip(axes, pokemon_names):
        counter = tracker.pokemon_move_counter[pokemon_name]
    ...
    fig.suptitle("Frequence d'utilisation des attaques par Pokemon", y=0.995)
...
def plot_tactical_understanding(env: PokemonEnv, plot_dir: str) -> None:
    ...
    values = [tracker.effectiveness_counter[k] for k in keys]
    effectiveness_colors = ["#4e9f3d", "#8f8f8f", "#d95f02"]

    bars = axes[0].bar(labels, values, color=effectiveness_colors)
    axes[0].set_title("Frequence super efficace / neutre / pas tres efficace")
    ...
    total_attack_actions = sum(tracker.effectiveness_counter.values())
    switch_count = tracker.switch_count
    item_count = tracker.item_count
    invalid_count = tracker.invalid_action_count
    ...
    axes[1].set_title("Usage des mecaniques de jeu")
    ...
    if tracker.matchup_move_counter:
        matchup_names = list(tracker.matchup_move_counter.keys())
        all_move_names = sorted({
            move_name
            for counter in tracker.matchup_move_counter.values()
            for move_name in counter.keys()
        })

        matrix = np.array(
            [
                [tracker.matchup_move_counter[matchup].get(move_name, 0) for move_name in all_move_names]
        ...
        axes[2].set_title("Utilisation des attaques par matchup")
        axes[2].text(0.5, 0.5, "Aucune donnee", ha="center", va="center")
        axes[2].axis("off")
...
def plot_terminal_state_summary(env: PokemonEnv, plot_dir: str) -> None:
    ...
    for side_label, infos_key, team_key in [
        ...
        for msg in history:
            ...
            for idx, pokemon in enumerate(team):
                ...
                grouped_stats = grouped[key]
                grouped_stats["episodes_seen"] += 1

                status = pokemon.get("status", "normal")
                hp_ratio = float(pokemon.get("hp_ratio", 0.0))

                grouped_stats["status_counter"][status] += 1
                grouped_stats["hp_ratio_sum"] += hp_ratio

                if idx == 0:
                    grouped_stats["front_end_count"] += 1

                if status == "KO":
                    grouped_stats["ko_count"] += 1
                else:
                    grouped_stats["alive_count"] += 1
                    grouped_stats["hp_ratio_alive_sum"] += hp_ratio
                    grouped_stats["alive_hp_samples"] += 1

        for (side, slot, name), grouped_stats in grouped.items():
            episodes_seen = max(1, grouped_stats["episodes_seen"])
            alive_pct = 100.0 * grouped_stats["alive_count"] / episodes_seen
            ko_pct = 100.0 * grouped_stats["ko_count"] / episodes_seen
            avg_hp_pct = 100.0 * grouped_stats["hp_ratio_sum"] / episodes_seen
            avg_alive_hp_pct = (
                100.0 * grouped_stats["hp_ratio_alive_sum"] / grouped_stats["alive_hp_samples"]
                if grouped_stats["alive_hp_samples"] > 0 else 0.0
            )
    ...
    rows_data.sort(key=lambda row: (row[0], int(row[1]), row[2]))
...
def plot(env: PokemonEnv, plot_dir: str) -> None:
